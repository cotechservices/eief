...
import {
  BookOpen, ShoppingCart, Package, Search, Plus, Trash2, Edit,
  Box, ImageIcon, X, Loader2, AlertCircle, FileText
} from "lucide-react";
...
export default function TransportLibrairiePage() {
  const [activeTab, setActiveTab] = useState<"articles" | "ventes">("articles");
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [dataArticles, dataVentes, dataEleves] = await Promise.all([
        fetchWithOffline('/api/admin/librairie/articles', 'articles'),
        fetchWithOffline('/api/admin/librairie/ventes', 'ventes'),
        fetchWithOffline('/api/admin/eleves')
      ]);
  ...
  const handlePrixChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setArticleData({ ...articleData, prix_unitaire: numericValue });
  };
  ...
  const filteredVentes = ventes.filter(v =>
    v.article_nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (v.eleve_nom && v.eleve_nom.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Link
            href="/dashboard/admin_transport/librairie/rapports"
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col">
        <div className="flex border-b flex-wrap">
          ...
          <Link href="/dashboard/admin_librairie/commandes" className="block">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  ...
                  <th className="px-6 py-3 text-center text-xs font-semibold uppercase">Stock</th>
                  ...
                  <tr key={a.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => openEditForm(a)} className="text-blue-600 hover:text-blue-800 p-1 transition" title="Modifier">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteArticle(a.id)} className="text-red-600 hover:text-red-800 p-1 transition" title="Supprimer">
          ...
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleArticleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" onClick={() => setShowArticleForm(false)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                  Annuler
                </button>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={handleVenteSubmit} className="space-y-4">
                ...
                >
                  ...
                  {articles.filter(a => a.quantite_stock > 0).map(a => (
                    <option key={a.id} value={a.id}>
                      {a.nom} - {formatPrix(a.prix_unitaire)} GNF (Stock: {a.quantite_stock})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" onClick={() => setShowVenteForm(false)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                  Annuler
                </button>
