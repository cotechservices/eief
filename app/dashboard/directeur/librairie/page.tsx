...
export default function LibrairiePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><div><h1 className="text-2xl font-bold">Librairie scolaire</h1><p className="text-gray-900">Vente fournitures, uniformes</p></div><button className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus className="w-4 h-4" />Ajouter un produit</button></div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-900">Produits</p><p className="text-2xl font-bold text-blue-600">{stats.totalProduits}</p></div><Package className="w-8 h-8 text-blue-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-900">Ventes (mois)</p><p className="text-2xl font-bold text-green-600">{stats.ventesMois.toLocaleString()} GNF</p></div><TrendingUp className="w-8 h-8 text-green-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-900">Valeur stock</p><p className="text-2xl font-bold text-orange-600">{stats.stockValeur.toLocaleString()} GNF</p></div><Package className="w-8 h-8 text-orange-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-900">Commandes</p><p className="text-2xl font-bold text-purple-600">{stats.commandesAttente}</p></div><ShoppingCart className="w-8 h-8 text-purple-200" /></div></div>
      </div>
