  ...
  interface Menu {
    ...
    prix: number;
    prix_annuel: number; // ⭐ Ajout du prix annuel
    inscrits: number;
    presents: number;
  }
  ...
  export default function CantinePage() {
    ...
    const [formData, setFormData] = useState({
      ...
      prix: 5000,
      prix_annuel: 2500000 // ⭐ Prix annuel par défaut
    });
    ...
    const handleOpenEdit = (menu: Menu) => {
      ...
      setFormData({
        ...
        prix: menu.prix || 5000,
        prix_annuel: menu.prix_annuel || 2500000
      });
      setShowForm(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const body = editingMenu ? { ...formData, id: editingMenu.id } : formData;
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {menus.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-gray-900">{new Date(m.date).toLocaleDateString()}</td>
                    ...
                    <td className="px-6 py-4 font-medium text-purple-600">
                      {m.prix_annuel ? `${m.prix_annuel.toLocaleString()} GNF` : "Non défini"}
                    </td>
            ...
            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md border border-gray-100 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-black">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-1">Prix (GNF)</label>
                    <input
                      required
                      type="number"
                      min="0"
                      ...
                      onChange={e => setFormData({ ...formData, prix_annuel: parseInt(e.target.value) || 0 })}
                      className="w-full border border-gray-300 p-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
