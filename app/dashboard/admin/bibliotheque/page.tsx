...
export default function BibliothequePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><div><h1 className="text-2xl font-bold">Bibliothèque</h1><p className="text-gray-500">Gestion des livres et emprunts</p></div><button className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus className="w-4 h-4" />Ajouter un livre</button></div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Total livres</p><p className="text-2xl font-bold text-blue-600">{stats.totalLivres}</p></div><Library className="w-8 h-8 text-blue-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Emprunts en cours</p><p className="text-2xl font-bold text-green-600">{stats.empruntsEnCours}</p></div><BookOpen className="w-8 h-8 text-green-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Livres en retard</p><p className="text-2xl font-bold text-red-600">{stats.livresRetard}</p></div><BookOpen className="w-8 h-8 text-red-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Membres actifs</p><p className="text-2xl font-bold text-purple-600">{stats.membresActifs}</p></div><Users className="w-8 h-8 text-purple-200" /></div></div>
      </div>

      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b flex justify-between items-center"><h3 className="font-semibold">Catalogue des livres</h3><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Rechercher..." className="pl-9 pr-4 py-1 border rounded-lg text-sm" /></div></div>
