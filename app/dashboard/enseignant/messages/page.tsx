...
export default function EnseignantMessagesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><div><h1 className="text-2xl font-bold">Messages</h1><p className="text-gray-900">Communication avec les parents</p></div>
      ...
      <div className="bg-white rounded-xl shadow-sm"><div className="border-b"><div className="flex gap-6 px-6"><button onClick={() => setActiveTab("recus")} className={`py-3 flex items-center gap-2 border-b-2 ${activeTab === "recus" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-900"}`}><Inbox className="w-4 h-4" />Reçus</button>
        <button onClick={() => setActiveTab("envoye")} className={`py-3 flex items-center gap-2 border-b-2 ${activeTab === "envoye" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-900"}`}><Send className="w-4 h-4" />Envoyés</button></div></div>
        <div className="divide-y">{filteredMessages.map((m) => (<div key={m.id} className="px-6 py-4 hover:bg-gray-50 flex justify-between items-center"><div><div className="flex items-center gap-2"><p className="font-medium">{activeTab === "recus" ? m.expediteur : m.destinataire}</p>{!m.lu && activeTab === "recus" && <span className="w-2 h-2 bg-blue-500 rounded-full"></span>}</div><p className="text-sm font-medium">{m.sujet}</p><p className="text-sm text-gray-900 line-clamp-1">{m.message}</p><p className="text-xs text-gray-900 mt-1">{m.date}</p></div>
