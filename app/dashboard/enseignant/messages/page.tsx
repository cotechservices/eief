...
export default function EnseignantMessagesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="divide-y">{filteredMessages.map((m) => (<div key={m.id} className="px-6 py-4 hover:bg-gray-50 flex justify-between items-center"><div><div className="flex items-center gap-2"><p className="font-medium">{activeTab === "recus" ? m.expediteur : m.destinataire}</p>{!m.lu && activeTab === "recus" && <span className="w-2 h-2 bg-blue-500 rounded-full"></span>}</div><p className="text-sm font-medium">{m.sujet}</p><p className="text-sm text-gray-500 line-clamp-1">{m.message}</p><p className="text-xs text-gray-400 mt-1">{m.date}</p></div>
