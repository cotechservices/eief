...
export default function EnseignantDevoirsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><div><h1 className="text-2xl font-bold">Devoirs</h1><p className="text-gray-900">Créez et gérez les devoirs</p></div>
      ...
      <div className="grid gap-4">{devoirs.map((d) => (<div key={d.id} className="bg-white rounded-xl shadow-sm p-4 hover:shadow-md transition"><div className="flex justify-between items-start"><div><h3 className="font-semibold text-gray-900">{d.titre}</h3><p className="text-sm text-gray-900">{d.matiere} - {d.classe}</p></div><div className="text-right"><p className="text-sm text-orange-600 flex items-center gap-1"><Clock className="w-3 h-3" />À rendre le {d.dateLimite}</p><p className="text-xs text-gray-900">{d.soumis}/{d.total} soumissions</p></div></div>
