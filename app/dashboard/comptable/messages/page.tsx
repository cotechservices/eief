  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm">
        ...
        <div className="divide-y">{filteredMessages.map((m) => (<div key={m.id} className="px-6 py-4 hover:bg-gray-50 flex justify-between items-center"><div><div className="flex items-center gap-2"><p className="font-medium">{activeTab === "recus" ? m.expediteur : m.destinataire}</p>{!m.lu && activeTab === "recus" && <span className="w-2 h-2 bg-blue-500 rounded-full"></span>}</div><p className="text-sm font-medium">{m.sujet}</p><p className="text-sm text-gray-500">{m.message.substring(0, 60)}...</p><p className="text-xs text-gray-400 mt-1">{m.date}</p></div><div className="flex gap-2"><button className="text-blue-600"><Eye className="w-4 h-4" /></button><button className="text-green-600"><Reply className="w-4 h-4" /></button><button className="text-red-600"><Trash2 className="w-4 h-4" /></button></div></div>))}</div>
