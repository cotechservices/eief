...
export default function CantinePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><div><h1 className="text-2xl font-bold">Gestion de la cantine</h1><p className="text-gray-500">Menus, réservations, présence</p></div><button className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus className="w-4 h-4" />Ajouter un menu</button></div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Inscrits cantine</p><p className="text-2xl font-bold text-blue-600">{stats.totalInscrits}</p></div><Users className="w-8 h-8 text-blue-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Moyenne/jour</p><p className="text-2xl font-bold text-green-600">{stats.moyenneJour}</p></div><Utensils className="w-8 h-8 text-green-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Recettes (mois)</p><p className="text-2xl font-bold text-orange-600">{stats.recettesMois.toLocaleString()} GNF</p></div><CreditCard className="w-8 h-8 text-orange-200" /></div></div>
        <div className="bg-white rounded-xl p-4"><div className="flex justify-between"><div><p className="text-gray-500">Taux présence</p><p className="text-2xl font-bold text-purple-600">{stats.tauxPresence}%</p></div><Users className="w-8 h-8 text-purple-200" /></div></div>
      </div>
