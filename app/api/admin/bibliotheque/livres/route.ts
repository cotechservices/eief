...
export async function POST(request: NextRequest) {
  try {
    ...
    const { titre, auteur, isbn, quantite, emplacement } = body;

    // par défaut disponible = quantite à la création
    const result = await query(`
      INSERT INTO livres_bibliotheque (titre, auteur, isbn, quantite, disponible, emplacement)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, [titre, auteur, isbn, quantite, quantite, emplacement]);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, titre, auteur, isbn, quantite, emplacement } = body;

    // Get current quantities to adjust disponible if quantite changes
    ...
    await query(`
      ...
      SET titre = $1, auteur = $2, isbn = $3, quantite = $4, disponible = $5, emplacement = $6
      WHERE id = $7
    `, [titre, auteur, isbn, quantite, newDisponible, emplacement, id]);
